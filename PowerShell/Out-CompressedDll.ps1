...
$AssemblyText = Out-CompressedDll -FilePath $FilePath

$Content = [IO.File]::ReadAllText("$(Split-Path -Path $MyInvocation.MyCommand.Path -Parent)\KeeThief.ps1")
$Content = $Content -replace "<REPLACE>",$AssemblyText
